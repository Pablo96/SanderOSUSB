...
void irq_ehci(){
	...
	for(int i = 0 ; i < portscount ; i++){
		...
		if(dtas&0x000FFF){
			...
			printf("EHCI: portcount #%x force reset\n",i);
			// now we tell how we want things...
			dtas = ((unsigned long*)valz)[0];
			dtas |= 0b111; 		 	// port on active
			...
			// wait a bit...
			printf("EHCI: portcount #%x wait\n",i);
			getch();
			printf("EHCI: portcount #%x endof wait\n",i);
			...
			printf("EHCI: portcount #%x end of reset\n",i);
...
void ehci_init(int bus,int slot,int function){
	...
	portscount = ((unsigned long*)HCSPARAMS)[0x00]&0b01111;
